...
class _BuyScreenState extends State<BuyScreen> {
  ...
  String _unitLabel(AppLocalizations loc, BuyUnit u) {
    switch (u) {
      case BuyUnit.gram:
        return loc.t("gram");
      case BuyUnit.tola:
  ...
  Map<String, double> _safeSilverMapBuy(dynamic metal) {
    try {
      final m = (metal as dynamic).silverQtyBuy;
      if (m is Map) {
        return m.map(
              (k, v) => MapEntry(
            k.toString(),
  ...
  double _buyUnitPriceGold(MetalPrice metal) {
    final perGram = (() {
      final k = _goldKarat;
      if (k != null) {
        final v = metal.goldKaratBuy[k];
        if (v != null && v > 0) return v;
      }
      return metal.buy.perGram;
    })();
  ...
  double _buyPriceSilverQty(MetalPrice metal, SilverQty q) {
    final map = _safeSilverMapBuy(metal);
    ...
    return metal.buy.perGram * _silverQtyGrams(q);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final selectedMetal =
    _metalCode == null ? null : _findMetal(latest.metals, _metalCode!);

    final unitBuy = (selectedMetal == null)
        ? 0.0
        : _isGold
        ? _buyUnitPriceGold(selectedMetal)
        : _buyPriceSilverQty(selectedMetal, _silverQty);

    final totalBuy = (selectedMetal == null)
        ? 0.0
        : _isGold
                      ...
                      steps: [
                        ...
                        Step(
                          ...
                          subtitle: Text(
                            _t(
                              settings,
                              "See BUY price instantly",
                              "فوراً بائے قیمت دیکھیں",
                            ),
                            style: TextStyle(
                              color: _accent.withOpacity(0.75),
                            ),
      ...
      children: [
        ...
        if (_isGold) ...[
          ...
          Wrap(
            ...
            children: [
              _ChoiceChip(
                label: _unitLabel(loc, BuyUnit.gram),
                selected: _unit == BuyUnit.gram,
                onTap: () => setState(() => _unit = BuyUnit.gram),
                accentColor: _accent,
              ),
              _ChoiceChip(
                label: _unitLabel(loc, BuyUnit.tola),
                selected: _unit == BuyUnit.tola,
                onTap: () => setState(() => _unit = BuyUnit.tola),
                accentColor: _accent,
              ),
            ],
          ),
